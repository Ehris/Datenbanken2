...
template<class T>
bool test_column(boost::shared_ptr<ColumnBaseTyped<T> > col, boost::shared_ptr<ColumnBaseTyped<T> > new_col, std::vector<T>& reference_data) {
	/****** BASIC INSERT TEST ******/
	...
	/****** STORE AND LOAD TEST ******/
	{
		...
		if (!equals(reference_data, new_col)) {
...
bool unittest(boost::shared_ptr<ColumnBaseTyped<int> > col) {
	...
	boost::shared_ptr<ColumnBaseTyped<int> > new_col (new Column<int>(col->getName(),INT));
	fill_column(col, reference_data);
	return test_column(col,new_col, reference_data);
}
...
bool unittest(boost::shared_ptr<ColumnBaseTyped<float> > col) {
	...
	boost::shared_ptr<ColumnBaseTyped<float> > new_col (new Column<float>(col->getName(),FLOAT));
...
bool unittest(boost::shared_ptr<ColumnBaseTyped<std::string> > col) {
	...
	boost::shared_ptr<ColumnBaseTyped<std::string> > new_col (new Column<std::string>(col->getName(),VARCHAR));
